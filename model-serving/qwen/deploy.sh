...
PROJECT=mm-test
GRANITE_DIR="$(cd "$(dirname "$0")/../granite-model" && pwd)"
...
oc project "${PROJECT}"
oc process -f "${GRANITE_DIR}/yaml/minio.yaml" \
...
git clone "https://memalhot:${ACCESS_TOKEN}@huggingface.co/Qwen/Qwen2.5-3B-Instruct"
...
python "${GRANITE_DIR}/python/create-bucket.py"
sleep 3
...
oc process -f "${GRANITE_DIR}/yaml/data-connection.yaml" \
  -p MINIO_ROOT_USER="${MINIO_ROOT_USER}" \
  -p MINIO_ROOT_PASSWORD="${MINIO_ROOT_PASSWORD}" \
...
oc process -f yaml/inference-service.yaml \
  ...
  -p MODEL_PATH="models/Qwen2.5-3B-Instruct" \
