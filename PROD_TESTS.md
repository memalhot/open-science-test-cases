...
- **chunked-prefill (t8) vs llm-d-local (t9)**, both 4 GPUs, same model and ramped load: t8 ~17.4k tok/s at ~192ms TTFT vs t9 ~11.5k tok/s at ~557ms TTFT, i.e. chunked delivers ~1.5x the throughput at ~3x lower TTFT. The workload is prefill-heavy (16,250 input tokens), so splitting the 4 GPUs into a 2-GPU prefill group + 2-GPU decode group, plus the KV-transfer step, makes the disaggregated path trail the single all-GPU TP group. Disaggregation's win case is decode-bound / high-concurrency serving, which this stress shape is not.
